(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('awesomoApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-awesomoApp-error');
                var entityKey = httpResponse.headers('X-awesomoApp-params');
                if (errorHeader) {
